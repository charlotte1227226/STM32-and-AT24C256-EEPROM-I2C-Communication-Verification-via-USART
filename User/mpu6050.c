/**
  ...
  * @brief   Source file of MPU6050 driver  
  ******************************************************************************
  */

#include "mpu6050.h"

IMU_Data_Frame imu_data_frame;
...
MPU6050_StatusTypeDef MPU6050_Init(IMU_Data_Frame *imu_data_frame)
{
  ...
  imu_data_frame->angle[0] = 0;
  imu_data_frame->angle[1] = 0;
  imu_data_frame->angle[2] = 0;

  if(I2C_Read(MPU6050_I2C_ID, MPU6050_ADDRESS, REG_WHO_AM_I, &rx_data, sizeof(rx_data)) != I2C_OK)
  {
  ...
  else
  {
    ...
    {
      printf("MPU6050 self test abnormal\n");
      //return MPU6050_ERROR;
    }
  ...
  while(1)
  {
    if(MPU6050_UpdateData(imu_data_frame) == MPU6050_DATA_READY)
    {
      printf("Roll: %.2f, Pitch: %.2f, Yaw: %.2f\n", imu_data_frame->angle[0], imu_data_frame->angle[1], imu_data_frame->angle[2]);
      if(fabsf(imu_data_frame->angle[0]) < 1 && fabsf(imu_data_frame->angle[1]) < 1)
      {
        break;
      }
    }
...
MPU6050_StatusTypeDef MPU6050_ResetAngleData(IMU_Data_Frame *imu_data_frame)
{
  imu_data_frame->current_timestamp = 0;
  imu_data_frame->last_timestamp = 0;
  imu_data_frame->delta_t = 0;
  imu_data_frame->angle[0] = 0;
  imu_data_frame->angle[1] = 0;
  imu_data_frame->angle[2] = 0;
  imu_data_frame->quat[0] = 1;
...
MPU6050_StatusTypeDef MPU6050_quat2euler(IMU_Data_Frame *imu_data_frame)
{
  float q1 = imu_data_frame->quat[0], q2 = imu_data_frame->quat[1], q3 = imu_data_frame->quat[2], q4 = imu_data_frame->quat[3];
  imu_data_frame->angle[0] = atan2(2 * (q1 * q2 + q3 * q4), 1 - 2 * (q2 * q2 + q3 * q3)) * 180 / M_PI;
  float temp = 2 * (q1 * q3 - q4 * q2);
  temp = temp > 1 ? 1 : temp;
  temp = temp < -1 ? -1 : temp;
  imu_data_frame->angle[1] = asin(temp) * 180 / M_PI;
  imu_data_frame->angle[2] = atan2(2 * (q1 * q4 + q2 * q3), 1 - 2 * (q3 * q3 + q4 * q4)) * 180 / M_PI;
  return MPU6050_OK;
}
...
MPU6050_StatusTypeDef MPU6050_UpdateData(IMU_Data_Frame *imu_data_frame)
{
  int status = MPU6050_GetDataStatus();
  if(status == MPU6050_DATA_READY)
  {
    MPU6050_ReadAccel(imu_data_frame);
    MPU6050_ReadGyro(imu_data_frame);
    MPU6050_UpdataTimestamp(imu_data_frame);
    MPU6050_UpdateOrientation(imu_data_frame);
    MPU6050_quat2euler(imu_data_frame);
  }
